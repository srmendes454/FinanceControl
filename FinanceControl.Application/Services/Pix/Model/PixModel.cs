...
namespace FinanceControl.Application.Services.Pix.Model
{
    [DataContract]
    [Table("Pix")]
    ...
    {
        #region [ Constructor ]
        public PixModel(Guid userId, string name, string linkedAccount, int expirationDay, PixType type, string color, PixWalletModel wallet)
        {
            PixId = Guid.NewGuid();
            CreatedBy = userId;
            Name = name;
        ...
        public void Update(string name, string linkedAccount, int expirationDay, PixType type, string color)
        {
            Name = name;
            LinkedAccount = linkedAccount;
            ExpirationDay = expirationDay;
